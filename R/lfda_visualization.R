...
plot.lfda <- function(x, labels, cleanText = FALSE, ...) {
  ...
  if (!is.factor(labels)) {
    stop("labels need to be an object convertable to a factor.")
  }

  if (length(labels) != dim(x$Z)[1]) {
    stop("length of labels differs from the number of rows in x$Z.")
  }

  if (!is.logical(cleanText)) {
    stop("cleanText needs to be TRUE(T) or FALSE(F).")
  }
  ...
  if (dim(x$Z)[2] > 3) {
    ...
    stop("dimensionality of the transformed data must be larger than 3.")
  }
  ...
  rgl::text3d(newData[, 2], newData[, 3], newData[, 4],
    col = Cols(transformedData$Class),
